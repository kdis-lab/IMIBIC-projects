...
#load the dataset
dataset <-read.csv(paste("factores-clinicos.csv", sep = ""), sep = ",", na.strings = "")

colnamesT <- colnames(dataset)[2:21]
...
#for each column of genetic factors
for(nameC in colnamesT){
  
  boxplot(dataset[,nameC])

  dev.copy(png,paste(output,nameC, ".png", sep = ""))
