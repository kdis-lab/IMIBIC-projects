...
public class AttSelectionClusterinClassification {

	public AttSelectionClusterinClassification(String datasetName, String algorithmName, double threshold, String type, String evaluationMode) {
		...
		try {
			...
			// Features
			ArrayList<String> lines = file.read(outputPath + "sortedfeatures.csv");
			
			//Remove the first line (head)
			lines.remove(0);
			...
			ArrayList<String> significantFactors = null;
			
			if(Files.exists(new File(outputPath + "SignificantFactors.txt").toPath())) {
				significantFactors = file.read(outputPath + "SignificantFactors.txt");
			}
				...
				for (int i = 0; i < indexes.length; i++) {
					...
					if (!summaries.contains(sG)) {
						...
						if (type.equals("classification")) {
							// run the clustering algorithm
							auc = evaluateModelClassification(algorithmName, datasetTemp, evaluationMode);
			...
			FileWriter writer = new FileWriter(
					outputPath + algorithmName + "/" + "allSignificantFactors.csv");

			writer.write(createCSV(summariesAllSignificants));

			writer.close();

			writer = new FileWriter(outputPath + algorithmName + "/" + "notAllSignificantFactors.csv");
	...
	public Summary summary(ArrayList<Integer> array, Instances dataset, double average,
		...
		if(significantFactors!=null)
		{	
			number = numberOfSignificants(atts, significantFactors);
		}
	...
	public double evaluateModelClassification(String algorithm, Instances dataset, String evaluationMode) throws Exception {
		...
		eval.crossValidateModel(algorithm, dataset, numFolds, new String[] { "" }, new Random());
	...
	public static void main(String args[]) throws NumberFormatException, Exception {
		...
		// threshold to filter
		double threshold = Double.parseDouble(Utils.getOption('t', args));
		
		//The possible values are 10Folds or LeaveOneOut
		...
		for (String dataset : datasets) {
			...
			for (String algorithm : algorithms) {
				...
				AttSelectionClusterinClassification sel;
				...
				sel = new AttSelectionClusterinClassification(dataset, algorithm, threshold, type, evaluationMode);
				n++;
			}
		}
	}
