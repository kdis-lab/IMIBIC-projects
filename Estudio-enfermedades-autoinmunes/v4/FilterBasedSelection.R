...
fileNames <- c("LinfoAnt-preproc","LinfoCMIT-preproc","LinfoComplicObstet-preproc","LinfodsDNA-preproc",
               ...
               "MonodsDNA-preproc", "MonoHta-preproc", "MonoTrombosis-preproc", "NeutroAnt-preproc","NeuroCMIT-preproc",
               "NeuroComplicObstet-preproc", "NeurodsDNA-preproc", "NeuroHta-preproc", "NeuroTrombosis-preproc")
...
for(fileName in fileNames){
  ...
  # For doing a LOOCV it is needed to set the parameter k equal to the number of samples in the dataset
  folds <- nrow(mydata)
  ntimes <- 1
  ...
  output <- paste("results/", fileName, sep = "")
