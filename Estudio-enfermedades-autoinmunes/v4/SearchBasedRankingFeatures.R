...
output <- "reports/enfermedades-autoinmunes/v4/with0s/"

threshold <- 0.9

plotROC <- TRUE

pathToDatasets <- "datasets/enfermedades-autoinmunes/v4/with0s/"

datasetNames <- c("LinfoAnt-preprocv2", "Mono-preprocv2", "NeutroAnt-preprocv2", 
                  "LinfoCMIT-preprocv2", "MonoCMIT-preprocv2", "NeutroCMIT-preprocv2", 
                  "LinfoComplicObstet-preprocv2", "MonoComplicObstet-preprocv2", "NeutroComplicObstet-preprocv2",
                  "LinfodsDNA-preprocv2", "MonodsDNA-preprocv2", "NeutrodsDNA-preprocv2",
                  "LinfoHta-preprocv2", "MonoHta-preprocv2", "NeutroHta-preprocv2",
                  "LinfoTrombosis-preprocv2", "MonoTrombosis-preprocv2", "NeutroTrombosis-preprocv2")
...
algorithms <- c("rf", "LogisticRegression")

#for each dataset
for(datasetName in datasetNames){
  
  #load the dataset and ranking of features
  dataset <- read.csv(paste(pathToDatasets, datasetName, ".csv", sep = ""), sep = ",")
  
  rankingFeature <- read.csv(paste(output, datasetName, "/sortedfeatures.csv", sep = ""), sep = "\t")
  ...
  # For each algorithm, execute the process
  for(algorithmName in algorithms){
    ...
    #######################
      ...
      for(feature in 1:nFeatures){
        ...
        for(otherFeature in feature:nFeatures)
        {
          ...
          if(algorithmName == "LogisticRegression"){
            ...
            if(algorithmName == "rf"){
              ...
              modelFit <- train(form, data = subdataset,
                                ...
                                metric = "ROC",
                                maximize = TRUE,
                                trControl = fitControl, tuneLength = 15)
          ...
          if(auc>= threshold){
            ...
            saveRDS(modelFit, paste(output, datasetName,"/", algorithmName, "-", nrow(df),".rds", sep = ""))
            ...
            if(plotROC){
              ...
              ggsave(width = 8, height = 5, filename = paste(output, datasetName,"/", algorithmName,"-", nrow(df),".png",sep = ""), bg = "transparent") 
      ...
      fileName <- paste(output, datasetName,"/", algorithmName, ".csv", sep = "")
